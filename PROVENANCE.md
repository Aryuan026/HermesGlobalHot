...
| Adapt | Home Global Hot owner projection, continuity sidecar binding, mouth snapshot, transport reconciliation, and post-LLM receipts | `runtime.py`, `request_projection.py`, `metadata.py`, `__init__.py` | Replaced Home surfaces with a Continuity source service, per-turn freeze, per-provider request carrier, final provider-body proof, and body-free SQLite delivery ledger. |
